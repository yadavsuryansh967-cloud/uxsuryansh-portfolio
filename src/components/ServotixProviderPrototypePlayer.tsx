...
const PROVIDER_STAGES: StageMarker[] = [
  { time: 0, label: 'Delivery & Service Log Matrix', stage: '01' },
  { time: 6, label: 'Filter Modal — Customer & Month (Rakesh / Aug 2026)', stage: '02' },
  { time: 10, label: '31-Day Customer Calendar & AI Prompt', stage: '03' },
  { time: 13, label: 'Payment Summary & Outstanding Collections (₹4,000)', stage: '04' },
  { time: 17, label: 'Verified Tax Invoice & QR Code (Rinku Singh)', stage: '05' },
  { time: 21, label: 'In-App Customer Chat & Verification (Raju Sharma)', stage: '06' },
  { time: 29, label: 'My Customer Directory & Subscription Status', stage: '07' },
  { time: 32, label: 'Customer Detail Spec — Rinku Singh (2L Daily)', stage: '08' },
  { time: 35, label: 'Add / Edit Customer Workflow & Form Inputs', stage: '09' },
  { time: 52, label: 'Provider Settings & Service Creation (Raju Sharma)', stage: '10' },
  { time: 76, label: 'Return to Delivery Dashboard & Service Matrix', stage: '11' },
];

export function ServotixProviderPrototypePlayer() {
  const [isPlaying, setIsPlaying] = useState<boolean>(true);
  const [isMuted, setIsMuted] = useState<boolean>(true);
  const [currentTime, setCurrentTime] = useState<number>(0);
  const [duration, setDuration] = useState<number>(82.0);
  ...
  const handleTimeUpdate = () => {
    if (videoRef.current) {
      ...
      // Find active stage
      let idx = 0;
      for (let i = PROVIDER_STAGES.length - 1; i >= 0; i--) {
        if (cur >= PROVIDER_STAGES[i].time) {
          idx = i;
          break;
        }
      }
  ...
  const handleLoadedMetadata = () => {
    if (videoRef.current) {
      setDuration(videoRef.current.duration || 82.0);
      setHasError(false);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-5 rounded-2xl bg-white/[0.02] border border-white/10">
        <div className="space-y-1">
          <div className="flex items-center space-x-2">
            <h4 className="font-editorial text-base text-white font-normal">
              Service Provider Prototype Walkthrough
            </h4>
            {hasError && (
          ...
          <p className="text-xs text-zinc-400">
            Interactive recording of the Service Provider user experience: daily multi-customer verification, customer filtering, 31-day logs, payment collections, verified tax invoicing, customer subscriptions, and provider settings.
          </p>
        </div>
        ...
        {!hasError && (
          <div className="flex items-center space-x-2 shrink-0">
            ...
            <button
              onClick={togglePlay}
              className="px-3.5 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-zinc-950 text-xs font-semibold flex items-center space-x-1.5 transition-colors shadow-sm"
            >
              {isPlaying ? (
                <>
      ...
      <div className="space-y-4">
        ...
        <div className="relative w-full rounded-2xl overflow-hidden bg-black border border-white/15 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.9)] group">
          ...
          <div className="relative w-full aspect-[1920/838] bg-black flex items-center justify-center">
            <video
              ref={videoRef}
              src="/assets/provider-prototype.mp4"
              autoPlay
              loop
              muted={isMuted}
              playsInline
            ...
            >
              <source src="/assets/provider-prototype.mp4" type="video/mp4" />
              <source src="/src/assets/provider-prototype.mp4" type="video/mp4" />
            </video>
            ...
            {hasError && (
              <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center bg-[#0C0D11] z-10 select-none">
                ...
                <h5 className="text-sm font-semibold text-white mb-2 font-sans">
                  Service Provider Prototype — Prototype unavailable to preview
                </h5>
                <p className="text-xs text-zinc-400 max-w-[260px] leading-relaxed">
                  The prototype video could not be loaded from <span className="font-mono text-[11px] text-zinc-300 bg-white/5 px-1 py-0.5 rounded">public/assets/provider-prototype.mp4</span>.
                </p>
              </div>
            )}
            ...
            <div className="p-4 bg-gradient-to-t from-black via-black/90 to-transparent border-t border-white/10 space-y-3">
              ...
              <div className="flex items-center space-x-3">
                <input
                  ...
                  value={currentTime}
                  onChange={handleSeek}
                  className="w-full h-1.5 bg-white/20 rounded-lg appearance-none cursor-pointer accent-amber-500 hover:bg-white/30 transition-colors"
                />
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3">
                  <span className="hidden sm:inline-flex items-center px-2.5 py-1 rounded-full bg-white/5 border border-white/10 text-xs text-zinc-300">
                    <span className="text-amber-400 font-semibold mr-1.5">{PROVIDER_STAGES[activeStageIndex].stage}</span>
                    <span className="truncate max-w-[280px]">{PROVIDER_STAGES[activeStageIndex].label}</span>
                  </span>
        ...
        {/* 11-Step Walkthrough Interactive Chapters */}
          ...
          <div className="lg:col-span-8 p-5 rounded-2xl bg-[#111216] border border-white/10 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[11px] text-zinc-400 font-medium">
                Chapter {activeStageIndex + 1} of {PROVIDER_STAGES.length}
              </span>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`text-left p-2.5 rounded-xl border transition-all flex items-center justify-between group ${
                      isActive 
                        ? 'bg-white/10 border-amber-500 text-white shadow-sm' 
                        : 'bg-white/[0.02] border-white/5 text-zinc-400 hover:text-zinc-200 hover:bg-white/5'
                    }`}
                  >
                    <div className="flex items-center space-x-2.5 min-w-0 pr-2">
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${
                        isActive ? 'bg-amber-500 text-zinc-950' : 'bg-white/5 text-zinc-400'
                      }`}>
                        {stg.stage}
                      </span>
                    ...
                    <span className={`text-[11px] shrink-0 font-medium ${isActive ? 'text-amber-400 font-semibold' : 'text-zinc-400'}`}>
                      {formatTime(stg.time)}
                    </span>
                  </button>
                );
          ...
          <div className="lg:col-span-4 p-5 rounded-2xl bg-[#111216] border border-white/10 space-y-3 text-xs">
            ...
            <ul className="space-y-2.5 text-zinc-400">
              <li className="flex items-start space-x-2">
                ...
                <span><strong className="text-zinc-200">Device Target:</strong> Android Compact (360 x 800) • 11 Chapters (~82s)</span>
              </li>
              <li className="flex items-start space-x-2">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0 mt-0.5" />
              ...
              <li className="flex items-start space-x-2">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0 mt-0.5" />
                <span><strong className="text-zinc-200">Coverage:</strong> Multi-customer delivery matrix, month filtering, 31-day history, payment analytics, tax invoices, resident chat, customer subscription management & provider profile settings</span>
              </li>
            </ul>
          </div>
        </div>
